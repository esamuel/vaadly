...
export const aiIntake = onRequest(async (req, res) => {
  try {
...
${text}`;
    ...
    const attachments = Array.isArray(parsed.attachments) ? parsed.attachments.filter((x: any) => typeof x === 'string') : [];
