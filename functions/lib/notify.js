...
exports.notify = (0, https_1.onRequest)(async (req, res) => {
    try {
...
async function notifyWorkOrderCreated(db, buildingId, data) {
...
async function notifyWorkOrderAssigned(db, buildingId, data) {
...
async function notifyWorkOrderCompleted(db, buildingId, data) {
    const { workOrderId, title, vendorName } = data;
    // Notify building members
    const membersQuery = await db.collection(`buildings/${buildingId}/members`)
        .where('role', 'in', ['committee', 'super_admin'])
...
async function notifyPaymentReceived(db, buildingId, data) {
...
async function notifyAnnouncement(db, buildingId, data, recipients) {
